...
abstract class RingBufferFields<E> extends RingBufferPad {
     ...
     *  BUFFER_PAD<<REF_ELEMENT_SHIFT 实际上就是BUFFER_PAD * scale，最终算出来REF_ARRAY_BASE就是基地址
     *  乘以4 = <<2 乘以8 = <<3
     */
    ...
    static {
        //arrayIndexScale方法获取的是数组类型的一个元素的引用大小，Object占用4个字节
        ...
        //buffer_pad。多少个元素占用128个字节的填充
        BUFFER_PAD = 128 / scale;
        // Including the buffer pad in the array base offset
        //arrayBaseOffset方法是一个本地方法，可以获取数组第一个元素的偏移地址。因为数组前面还有数组头相关内容。128即是填充
        //REF_ARRAY_BASE就是读第一个元素的开始位置
    ...
    RingBufferFields(
        ...
        /*
         * 申请的数组 entries 实际大小为 bufferSize + 2 * BUFFER_PAD，
         * BUFFER_PAD 个数组元素占用 128 字节，也就是说在数组前后各加了 128 字节的填充，这主要是为了防止伪共享。
         */
    ...
    /**
     * 填充bufferSize。其实是填充从buffer_pad后面的数组才是真正的值。细节：从BUFFER_PAD开始
     * @param eventFactory eventFactory。需要用于自行实现
     */
...
public final class RingBuffer<E> extends RingBufferFields<E> implements Cursored, EventSequencer<E>, EventSink<E> {
     ...
     * 从RingBuffer中获取一个可用的序号
     * 从用MultiProducerSequencer或者SingleProduerSequencer中获取下一个可用的序号
     */
    @Override
    public long next()
    {
    ...
    /**
     * translateAndPublis其实就是结合了next操作和publish
     * @param translator 用户自行实现的translator
     * @param sequence 序号
     */
